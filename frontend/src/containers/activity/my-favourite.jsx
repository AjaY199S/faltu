...
export default class MyFvourite extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      record: [],
  ...
  componentWillMount = async (req, res) => {
  ...
  getFavorite = async () => {
    ...
    await UserService.getFavorite(filter)
      .then((response) => {
        let responseData = response.data;
        if (responseData.success) {
          this.setState({
          ...
          responseData.data.map((data) => {
            ...
            if (
              data.favoriteUserId.lastActive &&
              data.favoriteUserId.lastActive != ""
            ) {
              let time =
                (new Date().getTime() -
                  new Date(data.favoriteUserId.lastActive).getTime()) /
                1000;
              let newTime = seconds_to_days_hours_mins_secs_str(time);
              data.favoriteUserId.lastActive = newTime;
                            ...
                            <div className="col-lg-4 col-md-3 text-center text-md-left border-right">
                              <figure className="mb-0 text-center">
                                <a
                                  onClick={(event) =>
                                    this.submitHandle(
                                      users.favoriteUserId.memberNumber,
                                      users.favoriteUserId._id
                                    )
                                  }
                                >
                                  <img
                                    ...
                                    src={
                                      users.favoriteUserId.profileImg
                                        ? SERVERURL +
                                          users.favoriteUserId.profileImg
                                        : IMG
                                    }
                                ...
                                <ul className="pl-0 mb-0">
                                  <li className=" username mt-2">
                                    <strong className="text-body font-weight-semibold">
                                      ...
                                      {users.favoriteUserId.firstName}{" "}
                                      {users.favoriteUserId.lastName}{" "}
                                    </strong>
                                  </li>
                                  ...
                                  <li className="common mb-2">
                                    ...
                                    {users.favoriteUserId.city
                                      ? users.favoriteUserId.city + ", "
                                      : ""}
                                    {users.favoriteUserId.province
                                      ? users.favoriteUserId.province + ","
                                      : ""}
                                    {users.favoriteUserId.country
                                      ? users.favoriteUserId.country
                                      : ""}
                                  </li>
                                  <li className="common mb-2">
                                    Last Active :{" "}
                                    {users.favoriteUserId.lastActive
                                      ? users.favoriteUserId.lastActive
                                      : ""}
                                  </li>

                                  <li className="common mb-2 msg">
                                    Received : {users.createdOn} ago
                                  </li>
                                </ul>
                              </div>
                            </div>
                            ...
                            <div className="icon">
                              {interestUserId.indexOf(
                                ...
                                <span
                                  ...
                                  title="Show Interest"
                                  onClick={(event) =>
                                    this.addInterest(users.favoriteUserId._id)
                                  }
                                >
                              ...
                              <span
                                className="trash"
                                onClick={(event) =>
                                  this.viewDetails(users.favoriteUserId._id)
                                }
                              ...
                              {favoriteUserId.indexOf(
                                ...
                                <span
                                  ...
                                  title="Add Favorite"
                                  onClick={(event) =>
                                    this.addFavorite(users.favoriteUserId._id)
                                  }
                                >
                                  <i className="icon-star"></i>
                                </span>
                              ...
                              <span
                                ...
                                data-target="#myModal"
                                onClick={(event) => {
                                  this.loadModal(users.favoriteUserId);
                                }}
                              >
